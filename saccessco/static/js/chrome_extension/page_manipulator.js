...
// Split-aware: date logic in window.skyscannerDates; location logic in window.skyscannerLocations.
...
(function (window) {
  ...
  function _robustClick(el) {
    if (!el) return false;
    var target = el;
    if (!target.matches('button,[role="button"],a,[role="tab"],[role="option"],[data-testid],.bpk-autosuggest__suggestion')) {
      var inner = el.querySelector('button,[role="button"],a,[role="tab"],[role="option"],.bpk-autosuggest__suggestion');
  ...
  // Turn a strict selector into tolerant alternatives (case-insensitive contains + text variants)
  function _textVariants(txt){
    var t = String(txt||'');
    var norm = t.replace(/\s+/g,' ').trim();
    var hyphen = norm.replace(/\s+/g,'-');
    var nospace = norm.replace(/\s+/g,'');
    var rmHyphenToSpace = t.replace(/-/g,' ').replace(/\s+/g,' ').trim();
    var rmHyphenNoSpace = t.replace(/-/g,'');
    var noParens = norm.replace(/\s*\([^)]*\)\s*/g,' ').replace(/\s+/g,' ').trim();
    var set = {}; set[t]=1; set[norm]=1; set[hyphen]=1; set[nospace]=1; set[rmHyphenToSpace]=1; set[rmHyphenNoSpace]=1; set[noParens]=1;
    return Object.keys(set);
  }
  function _selectorCandidates(sel){
    var candidates = [sel];
    // Make [aria-label*="..."] / [title*="..."] case-insensitive by adding the CSS4 " i" flag
    var insensitive = sel.replace(/\[(aria-label|title|aria-labelledby)\*\s*=\s*(["'])([^"']+)\2\]/gi, function(_,attr,q,val){
      return '['+attr+'*='+q+val+q+' i]';
    });
    if (insensitive !== sel) candidates.push(insensitive);

    // Expand value variants for the first *-contains token we find
    var m = sel.match(/\[(aria-label|title|aria-labelledby)\*\s*=\s*(["'])([^"']+)\2\]/i);
    if (m){
      var attr=m[1], q=m[2], val=m[3];
      var vals = _textVariants(val);
      for (var i=0;i<vals.length;i++){
        candidates.push(sel.replace(m[0], '['+attr+'*='+q+vals[i]+q+' i]'));
      }
    }

    // De-dupe
    var seen={}, out=[]; for (var j=0;j<candidates.length;j++){ var c=candidates[j]; if(!seen[c]){ seen[c]=1; out.push(c); } }
    return out;
  }
  ...
  function _fallbackClickByText(text) {
    ...
    var nodes = Array.prototype.slice.call(document.querySelectorAll('button,[role="button"],[role="tab"],[data-testid],li,span,a,[role="option"]')).filter(_visible);
  ...
  // Flights tab helpers
  function _findFlightsTab(){
    var nodes = Array.prototype.slice.call(document.querySelectorAll("button[role='tab'],[role='tab'],button,[data-testid]"))
      .filter(_visible);
    for (var i=0;i<nodes.length;i++){
      ...
      if (/flights/i.test(title) || /flights/i.test(testid) || /\bflights\b/i.test(txt)) return el;
    }
    return null;
  }
  function _isFlightsSelector(sel){ return /\bflights\b/i.test(String(sel||'')); }
  ...
  async function _ensureDateVisibleIfNeeded(selector) {
    ...
    try {
      ...
      console.warn('[pageManipulator] date helper failed for', selector, e);
    }
  }

  /* ---------------- Locations: via window.skyscannerLocations ---------------- */
  function _isOriginButtonSelector(s) {
    if (!s) return false; var x = String(s);
    return /\borigin\b/i.test(x) || /\bflying\s+from\b/i.test(x) || /\bfrom\b/i.test(x) || /data-testid\s*=\s*["']?origin/i.test(x) || /\boriginInput\b/.test(x) || /\baria-label\s*=\s*["']from["']/i.test(x);
  }
  function _isDestinationButtonSelector(s) {
    if (!s) return false; var x = String(s);
    return /\bdestination\b/i.test(x) || /\bflying\s+to\b/i.test(x) || /\bto\b/i.test(x) || /data-testid\s*=\s*["']?destination/i.test(x) || /\bdestinationInput\b/.test(x);
  }
  function _looksLikeLocationStep(step) {
    if (!step) return false; var sel = String(step.selector || '');
    if (step.action === 'click' && (_isOriginButtonSelector(sel) || _isDestinationButtonSelector(sel))) return true;
    if (step.action === 'typeInto' && /(input\[role=['"]combobox['"]]|input\[type=['"](search|text)['"]]|#(origin|destination)Input-input)/i.test(sel)) return true;
    if ((step.action === 'waitForElement' || step.action === 'click') && (/(\[role=['"]listbox['"]\])/i.test(sel) || /(originInput|destinationInput)-menu/.test(sel))) return true;
    if (step.action === 'click' && /\(([A-Z]{3})\)/.test(sel)) return true; // IATA
    if (step.action === 'click' && /\baria-label=\s*['"]From['"]/i.test(sel)) return true;
    return false;
  }
  async function _tryHandleLocationSequence(plan, parameterManagerInstance) {
    ...
    var textParamName = null;  // parameter name to read
  ...
  var pageManipulator = {
    _findElementAndAssertVisible: async function (selector, timeoutMs, checkIntervalMs) {
      ...
      // Dates: attempt to reveal target cell first
    ...
    executePlan: async function (plan, parameters) {
      ...
      try {
        ...
        // 1) LOCATION fast-path (do NOT return early; skip redundant steps later)
        ...
        // 2) If any step implies a return action, proactively ensure RETURN mode
        try {
        ...
        for (var i = 0; i < plan.length; i++) {
          ...
          if (step.action === 'click' && (_isFlightsSelector(step.selector) || /\baria-label=\s*['"]Flights['"]/i.test(String(step.selector)))) {
            ...
            if (flightsTab) {
              if (String(flightsTab.getAttribute('aria-selected')||'').toLowerCase()==='true') {
                individualActionResults.push({ action: step.action, selector: step.selector + ' (already selected)', value: null, success: true });
                await _sleep(30);
                continue;
              }
              _robustClick(flightsTab);
              individualActionResults.push({ action: step.action, selector: step.selector + ' (fallback flights tab)', value: null, success: true });
              await _sleep(120);
              continue;
            }
          }
          ...
          if (step.action === 'waitForElement') {
            ...
            if (!element) {
              // Fallback: try clicking by visible text contained in aria-label
        ...
        return {
          status: "failed",
          results: individualActionResults.length ? individualActionResults : [{ success: false, error: "Top-level execution error: " + (topLevelError && topLevelError.message ? topLevelError.message : "Unknown") }]
        };
      }
    }
  };
  ...
  console.log("Page Manipulator module loaded (tolerant selectors + flights tab fallback).");
})(window);
